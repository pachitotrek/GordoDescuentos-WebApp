...
@Component({
  selector: 'app-addloca',
  templateUrl: './addloca.page.html',
  styleUrls: ['./addloca.page.scss'],
})
export class AddlocaPage implements OnInit {
  ...
  crear(){
    this.showLoader();
    this.api.crearLocation(this.form.value).subscribe((resp:any)=>{
        if(resp.ok){
          let categoria="lugar";        
          let id=resp.locationdb._id;
          if(this.archivo){
  ...
  get(){
    this.api.GetLocations().subscribe((resp:any)=>{
        if(resp.ok){
          this.categorias=resp.locationdb;  
        }
    });
  }
  setMenu(data){
  ...
  updateCat(){
    this.showLoader();
    this.api.updateLocation(this.form.value).subscribe((resp:any)=>{
      if(resp.ok){
        let categoria="lugar";        
  ...
  eliminar(id){
    this.api.deleteLocation(id).subscribe((resp:any)=>{
      if(resp){    
        this.get();
      }
    });
